...
- Name: new-story.spec.ts >> serves new-story.html through the proxy
- Location: test/e2e/new-story.spec.ts:24:1
...
Call log:
  - navigating to "/new-story.html", waiting until "domcontentloaded"
...
> 25  |   await page.goto('/new-story.html', {
  ...
  117 |   expect(submitNewStoryUrl, 'submitNewStoryUrl from config').toBeTruthy();
  118 | 
  119 |   if (!submitNewStoryUrl) {
  120 |     throw new Error('Expected submitNewStoryUrl in config.json response');
  121 |   }
  122 | 
  123 |   await expect(page.locator('form')).toHaveAttribute(
  124 |     'action',
  125 |     submitNewStoryUrl,
```
 ...
 * @returns {storage is { bucket: (...args: unknown[]) => unknown }} True when storage.bucket is callable.
 */
function hasBucketFunction(storage) {
  return Boolean(
    storage &&
      typeof (/** @type {{ bucket?: unknown }} */ (storage).bucket) ===
        'function'
  );
}
...
function hasValidGrandparentChain(ref) {
  return (
    hasParentRef(ref) &&
    hasGrandparentFromParent(/** @type {{ parent?: unknown }} */ (ref).parent)
  );
}

...
export function delivery(request) {
  const duration = request?.deliveryDurationSeconds;
  const end = pointTimestamp(request?.deliveryPoint);
...
export function procurement(request) {
  ...
  const latestEnd = Math.min(deliveryStart, supplierEnd);
  const candidate = latestPlacement(
    duration,
    request?.nowTimestamp,
    iso(latestEnd)
...
export function pickup(request) {
  const duration = request?.pickupDurationSeconds;
  if (!finiteDuration(duration))
    return { feasible: false, reason: 'invalid-duration' };

...
export function createRealtimeVoicePrototypeElement(inputString, dom) {
  ...
  appendDebugLog(controls, 'Toy controls mounted. No audio or SDP is logged.', dom);
...
function parseConfig(inputString) {
  try {
    ...
    return {
      ...
      description: stringOr(parsed.description, 'OpenAI Realtime WebRTC voice toy.'),
...
async function connectRealtimeVoice(state, controls, dom) {
  ...
  try {
    ...
    disconnectRealtimeVoice(state, controls, 'Connection cleanup complete.', dom);
...
async function startPeerConnection(state, controls, dom) {
  ...
  const answerSdp = await requestRealtimeAnswer(offer.sdp ?? '', controls.endpoint);
  await peerConnection.setRemoteDescription({ type: 'answer', sdp: answerSdp });
  appendDebugLog(controls, `Applied SDP answer from ${controls.serverLabel}.`, dom);
}
...
function createDebugDataChannel(state, peerConnection, controls, dom) {
  ...
  dataChannel.addEventListener('message', event => {
    appendDebugLog(controls, `Event: ${summarizeRealtimeEvent(event.data)}`, dom);
  });
...
function wirePeerConnectionEvents(peerConnection, controls, dom) {
  peerConnection.addEventListener('connectionstatechange', () => {
    appendDebugLog(controls, `Peer state: ${peerConnection.connectionState}.`, dom);
  });
  peerConnection.addEventListener('iceconnectionstatechange', () => {
    appendDebugLog(controls, `ICE state: ${peerConnection.iceConnectionState}.`, dom);
  });
}
...
function toggleMute(state, controls, dom) {
  ...
  appendDebugLog(controls, state.muted ? 'Microphone muted.' : 'Microphone live.', dom);
}

...
describe('getEnvHelpers', () => {
  ...
  test('returns each required helper when all are functions', () => {
    ...
    expect(requireEnvHelper(new Map([['helper', helpers.getUuid]]), 'helper')).toBe(helpers.getUuid);
  });
});
...
describe('ensureDend2', () => {
  ...
  test('migrates valid legacy data and replaces invalid structures', () => {
    ...
    ensureDend2(invalid);
    expect(invalid.temporary.TRAN1).toEqual({ stories: [], pages: [], options: [] });
    const empty = {};
    ensureDend2(empty);
    expect(empty.temporary.TRAN1).toEqual({ stories: [], pages: [], options: [] });
  });
});
...
describe('browser toy value helpers', () => {
  test('validates records and parses with fallbacks', () => {
    ...
    expect(parseJsonOrFallback('bad', { fallback: true })).toEqual({ fallback: true });
    expect(isPlainObject({})).toBe(true);
    expect(isPlainObject([])).toBe(false);
    expect(isPlainObject(null)).toBe(false);
    expect(isPlainObject(42)).toBe(false);
    expect(toRecordOrNull({ id: 1 }, value => isPlainObject(value))).toEqual({ id: 1 });
    expect(toRecordOrNull('no', value => isPlainObject(value))).toBeNull();
    expect(createOptions({ firstOption: 'One', secondOption: '', thirdOption: 3 }, () => 'id', 'page'))

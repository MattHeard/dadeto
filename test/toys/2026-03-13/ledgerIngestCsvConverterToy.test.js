...
describe('ledgerIngestCsvConverterToy', () => {
  ...
  it('normalizes date, amount, and text helper boundaries directly', () => {
    expect(
      ledgerIngestCsvConverterToyTestOnly.isCsvLineBreakCharacter('\n')
    ).toBe(true);
    expect(
      ledgerIngestCsvConverterToyTestOnly.isCsvLineBreakCharacter('x')
    ).toBe(false);
    expect(ledgerIngestCsvConverterToyTestOnly.isCsvCarriageReturn('\r')).toBe(
      true
    );
    expect(ledgerIngestCsvConverterToyTestOnly.isCsvCarriageReturn('\n')).toBe(
      false
    );
    expect(ledgerIngestCsvConverterToyTestOnly.isCsvEscapedQuote('"')).toBe(
      true
    );
    expect(ledgerIngestCsvConverterToyTestOnly.isCsvEscapedQuote('x')).toBe(
      false
    );
    expect(
      ledgerIngestCsvConverterToyTestOnly.hasPendingCsvParseData({
        cell: '',
        row: [],
      })
    ).toBe(false);
    ...
    expect(ledgerIngestCsvConverterToyTestOnly.isBlankLedgerCsvRow([''])).toBe(
      true
    );
    expect(ledgerIngestCsvConverterToyTestOnly.isBlankLedgerCsvRow(['x'])).toBe(
      false
    );
    expect(
      ledgerIngestCsvConverterToyTestOnly
        .buildHeaderLookup([' Amount '])
        .get('Amount')
    ).toBe(0);
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate('31.12.2025')
    ).toBe('2025-12-31');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate('2025-12-31')
    ).toBe('');
    expect(ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate(null)).toBe('');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate('1.12.2025')
    ).toBe('');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate('31.12.2025x')
    ).toBe('');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvDate('x31.12.2025')
    ).toBe('');
    expect(
      ledgerIngestCsvConverterToyTestOnly.parseCsvDateMatch(null)
    ).toBeNull();
    expect(
      ledgerIngestCsvConverterToyTestOnly.parseCsvDateMatch(' 31.12.2025 ')
    ).not.toBeNull();
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvAmount('1.234,50 EUR')
    ).toBe('1234.5');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvAmount('1.234')
    ).toBe('1234');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvAmount('not-a-number')
    ).toBe('');
    expect(ledgerIngestCsvConverterToyTestOnly.normalizeCsvAmount(null)).toBe(
      ''
    );
    expect(ledgerIngestCsvConverterToyTestOnly.parseCsvAmount('1.234')).toBe(
      1234
    );
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvTextCandidate('  hello  ')
    ).toBe('hello');
    expect(
      ledgerIngestCsvConverterToyTestOnly.normalizeCsvTextCandidate(null)
    ).toBe('');
  });

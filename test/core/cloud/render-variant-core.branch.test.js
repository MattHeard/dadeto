...
describe('renderVariantCoreTestUtils', () => {
  ...
  test('covers option href and pending-path fallbacks', () => {
    ...
    expect(utils.readSnapshotData({ data: () => null })).toEqual({});
    expect(utils.readSnapshotData({ data: () => ({ value: 1 }) })).toEqual({ value: 1 });
    ...
    expect(
      utils.resolveIncomingParentRef({ incomingOption: 'options/1' }, {
        doc: jest.fn(() => optionRef),
      })
    ).toBe(ancestor);
    expect(utils.resolveOptionHref('slug', {})).toBe('../new-page.html?option=slug');
    expect(utils.resolveOptionHref('slug', { targetPageNumber: 7 })).toBe('/p/7.html');
    expect(
      utils.resolveOptionHref('slug', { targetPageNumber: 7 }, { rewriteTargetPageNumbers: [7] })
    ).toBe('../new-page.html?page=7');
    expect(utils.buildOptionsItems({ pageNumber: 1, variantName: 'a', options: [] })).toBe('');
    expect(utils.resolveStoryIdFromPath({ ref: { path: 'stories/story-1/pages/2' } })).toBe('story-1');
    expect(utils.resolveStoryIdFromPath({ ref: {} })).toBeUndefined();
    expect(utils.resolveVariantIdFromPath({ ref: { path: 'stories/1/pages/2/variants/v1' } })).toBe('v1');
  ...
  test('covers tree visibility propagation fallbacks and parent updates', async () => {
    ...
    await utils.updateTreeVisibilityForVariantChange({ change: skipped, db: {} });
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { data: () => ({ visibility: 1 }) }, before: { exists: false } },
      db: {},
    });
    await utils.updateTreeVisibilityForVariantChange({
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: {
        after: {
          ...
          data: () => ({ visibility: 1, treeVisibilitySum: 1, incomingOption: 'options/o1' }),
        },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/no-data' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/null-data' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
      db: { doc: jest.fn(() => nullDataRef) },
    });
    const nullSumRef = {
      get: jest.fn().mockResolvedValue({ exists: true, data: () => ({ treeVisibilitySum: null }) }),
      update: jest.fn().mockResolvedValue(undefined),
    };
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/null-sum' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/missing' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/null-snapshot' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/null-data-method' }, data: () => ({ visibility: 1 }) }, before: { exists: false } },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: {
        after: { ref: { path: 'variants/no-change' }, data: () => ({ visibility: 1 }) },
    ...
    await utils.updateTreeVisibilityForVariantChange({
      change: {
        after: { ref: { path: 'variants/empty-parent' }, data: () => ({ visibility: 1, incomingOption: 'options/o2' }) },
        before: { exists: false },
      },
      db: {
        doc: jest.fn(path => path === 'options/o2' ? { parent: { parent: { get: jest.fn().mockResolvedValue({ data: () => undefined }), update: jest.fn() } } } : {
          get: jest.fn().mockResolvedValue({ exists: true, data: () => undefined }),
          update: jest.fn(),
        }),
      },
    });
    const missingParentData = { get: jest.fn().mockResolvedValue({}), update: jest.fn() };
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/missing-parent-data' }, data: () => ({ visibility: 1, incomingOption: 'options/o3' }) }, before: { exists: false } },
      db: {
        doc: jest.fn(path => path === 'options/o3' ? { parent: { parent: missingParentData } } : {
          get: jest.fn().mockResolvedValue({ exists: true, data: () => ({}) }),
          update: jest.fn(),
        }),
      },
    });
    const nullParentData = { get: jest.fn().mockResolvedValue({ data: null }), update: jest.fn() };
    await utils.updateTreeVisibilityForVariantChange({
      change: { after: { ref: { path: 'variants/null-parent-data' }, data: () => ({ visibility: 1, incomingOption: 'options/o4' }) }, before: { exists: false } },
      db: {
        doc: jest.fn(path => path === 'options/o4' ? { parent: { parent: nullParentData } } : {
          get: jest.fn().mockResolvedValue({ exists: true, data: () => ({}) }),
          update: jest.fn(),
        }),
      },
    });
    expect(utils.buildOptionsItems({
      pageNumber: 1,
      variantName: 'a',
  ...
  test('clears the rendered variant tree-weight dirty marker', async () => {
    ...
    await renderVariantCoreTestUtils.persistRenderPlan({
      snap,
      context: undefined,
      db: { doc: jest.fn(() => snapRef), collection: jest.fn(path => path === variantsRef.path ? variantsRef : variantsRef) },

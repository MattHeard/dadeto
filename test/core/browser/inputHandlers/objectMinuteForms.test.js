...
function createDom() {
  return {
    createElement: jest.fn(tag => {
      ...
      node.querySelectorAll = jest.fn(selector => selector === 'input'
        ? node.children.map(row => row.children?.[1]).filter(Boolean)
        : []);
      return node;
    }),
    setClassName: jest.fn((node, value) => { node.className = value; }),
    setTextContent: jest.fn((node, value) => { node.textContent = value; }),
    appendChild: jest.fn((parent, child) => { parent.children.push(child); return child; }),
    getValue: jest.fn(node => node.value),
    setValue: jest.fn((node, value) => { node.value = value; }),
...
describe('object-minute structured input handlers', () => {
  test('renders an asset form, restores values, and serializes edits', () => {
    ...
    const textInput = { value: JSON.stringify({ assetId: 'A-1', resetRequired: false }) };
    ...
    expect(dom.createElement.mock.calls.map(([tag]) => tag)).toEqual([
      'div', 'label', 'span', 'input', 'label', 'span', 'input',
    ...
    expect(form.children.map(row => [row.children[0].textContent, row.children[1].type])).toEqual([
      ['Asset ID', 'text'], ['SKU', 'text'], ['Name', 'text'],
      ['Storage location', 'text'], ['Condition', 'text'], ['Availability', 'text'],
      ['Owner', 'text'], ['Reset required', 'checkbox'], ['Notes', 'text'],
    ]);
    ...
    expect(form.children.slice(0, 7).every(row => !Object.hasOwn(row.children[1], 'checked'))).toBe(true);
    expect(Object.hasOwn(form.children[8].children[1], 'checked')).toBe(false);
    expect(form.children.slice(1).filter(row => row.children[1].value === '').length).toBe(8);
    expect(JSON.parse(textInput.value)).toEqual({
      assetId: 'A-1', sku: '', name: '', storageLocation: '', condition: '',
      availability: '', owner: '', resetRequired: false, notes: '',
    });
    ...
    expect(trueContainer.children[0].children[7].children[1].checked).toBe(true);
  });
  ...
  test('serializes number fields as numbers and blank numbers as null', () => {
    ...
    const form = container.children[0];
    expect(form.children.map(row => [row.children[0].textContent, row.children[1].type])).toEqual([
      ['SKU', 'text'], ['Delivery latitude', 'number'], ['Delivery longitude', 'number'],
      ['Delivery time (UTC)', 'text'], ['Pickup latitude', 'number'],
      ['Pickup longitude', 'number'], ['Pickup time (UTC)', 'text'],
    ]);
    ...
    expect(JSON.parse(textInput.value)).toEqual({
      sku: '', deliveryLocation: { lat: 52.5, lon: null },
      deliveryTime: '2026-08-22T12:00', pickupLocation: { lat: null, lon: null }, pickupTime: '',
    });
  });

...
describe('notion codex config', () => {
  ...
  test('normalizes invalid values and nested overrides', () => {
    ...
    expect(config.notion.taskContext).toBe(
      DEFAULT_NOTION_CODEX_CONFIG.notion.taskContext
    );
    ...
    expect(config.pollIntervalMs).toBe(
  ...
  test('loads configured JSON and uses missing-file fallback', async () => {
    const readFileImpl = jest.fn(async () =>
      JSON.stringify({ maxConcurrentRuns: 3 })
    );
    await expect(
  ...
  test('accepts omitted load options', async () => {
    await expect(loadNotionCodexConfig()).rejects.toThrow(

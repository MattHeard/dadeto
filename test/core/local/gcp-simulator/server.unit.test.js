...
describe('gcp simulator server without a listener', () => {
  test('registers and executes simulator routes with injected dependencies', async () => {
    const routes = new Proxy(
      {
        getApiKeyCreditV2: jest.fn(async () => ({ status: 200, body: { credits: 1 } })),
        submitNewStory: jest.fn(async () => ({ status: 201, body: {} })),
      ...
      {
        get(target, name) {
          if (!target[name]) target[name] = jest.fn(async () => ({ status: 200, body: {} }));
          return target[name];
        },
      }
    );
    ...
    const app = {
      ...
      get: jest.fn((path, handler) => routeHandlers.push({ method: 'get', path, handler })),
      post: jest.fn((path, handler) => routeHandlers.push({ method: 'post', path, handler })),
    ...
    const submitHandler = routeHandlers.find(route => route.path === '/__sim/submit-new-story').handler;
    await submitHandler({ ...request, headers: {}, get: () => undefined }, makeResponse(), jest.fn());

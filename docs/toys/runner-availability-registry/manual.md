...
JSON object containing a `runners` array. Submit a JSON value in the toy's input area. A minimal example is:
...
JSON object containing normalized `runners` and a `summary`. The result is returned as JSON or rendered by the toy's configured presenter. A representative result is:
...
In scope: normalize multiple runners and their availability windows. Out of scope: jobs, routing, travel time, matching, persistence, and optimization.

### Limits and assumptions

Availability windows use caller-provided time strings. The toy is synchronous, pure, and deterministic.

...
Provide an original frame-stepped paddle game that persists state across submits and renders entirely through canvas shapes. Exercise the existing keyboard-capture input path while keeping room for generic Joy-Con/gamepad-style raw input handling inside the toy.

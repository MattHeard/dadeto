...
JSON object containing an `allocations` array. Submit a JSON value in the toy's input area. A minimal example is:
...
JSON object containing normalized `allocations` and a `summary`. The result is returned as JSON or rendered by the toy's configured presenter. A representative result is:
...
In scope: normalize allocations, transport-inclusive windows, status, and deterministic ordering. Out of scope: request references, runner assignment, route planning, persistence, and conflict detection.

### Limits and assumptions

`possessionContextId` is the shared relationship key. `allocatedFrom` and `allocatedTo` cover transport, possession, and return. The toy is synchronous, pure, and deterministic.

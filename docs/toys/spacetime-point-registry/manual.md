...
JSON object containing a `points` array. Submit a JSON value in the toy's input area. A minimal example is:
...
normalized `points` and a `summary` count. The result is returned as JSON or rendered by the toy's configured presenter. A representative result is:
...
In scope: point IDs, WGS84 latitude/longitude, and UTC-minute timestamps. Out of scope: persistence, feasibility, segments, routing, and rental-domain relationships.

### Limits and assumptions

Coordinates follow POSS1 semantics and are rounded to six decimals. Times follow POSS1’s `YYYY-MM-DDTHH:MMZ` UTC-minute contract. Altitude is intentionally deferred.

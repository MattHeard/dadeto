...
- **Branch coverage:** Added a guard test that calls `createHandleRequest()` without any configuration so the default `options = {}` path is executed and the file’s branch coverage returns to 100%.

...
- **Action:** Removed the default argument values so that each helper now requires the caller to pass an environment object explicitly; this keeps the contract clear and avoids silently reading global state inside the helper.

...
- Chosen fix: replaced the `--full-auto` alias with one shared Ralph default arg list that explicitly uses `--model gpt-5-mini`, `--sandbox workspace-write`, and `--ask-for-approval never`, then updated focused launcher/config tests to lock that contract down.

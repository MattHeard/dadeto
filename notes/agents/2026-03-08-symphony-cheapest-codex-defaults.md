...
- Chosen fix at that stage: replaced the `--full-auto` alias with one shared Ralph default arg list that explicitly used `--model gpt-5-mini` and `--sandbox workspace-write`, then updated focused launcher/config tests to lock that contract down.

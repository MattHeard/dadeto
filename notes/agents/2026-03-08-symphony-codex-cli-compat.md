...
- Change:
  ...
  - kept the bounded launcher defaults otherwise unchanged at that point: `exec`, `--skip-git-repo-check`, `--model gpt-5-mini`, and `--sandbox workspace-write`
...
- Follow-up:
  - this bead only fixed CLI argument compatibility; later slices can address model/account compatibility, completion reconciliation, or any next-stage runtime blocker that appears after Codex starts successfully

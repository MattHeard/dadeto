...
Chosen fix: I split the parser and lookup flow into smaller helpers, added explicit edge-case tests for blank input, missing paths, undefined values, and thrown string errors, and then rebuilt the generated blog assets so `MEMO1` landed in `public/blog.json`.

...
- **Action:** Removed the optional message arguments, dropped the validation guard clauses for `verifyToken`, `isAdminUid`, and the response helpers, removed the fallback default argument so callers must supply the dependency object, extracted the token parsing logic into its own helper, and updated the tests to reflect the streamlined middleware contract.

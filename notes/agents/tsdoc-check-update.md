...
- **Quality-gate policy note**: `tsdoc:check` is still backlog-heavy enough that it should stay outside the default local aggregate gate. Prefer keeping it as an explicit follow-up command for typing-focused beads while `npm run check` / `npm run check:full` cover the default local validation path.
